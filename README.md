...
```
Create a RenderConfig from a resolution, and import a Mesh instance from a 3D file format:
```uiua
M~RenderConfig 500_500
°M~OBJ &fras "assets/suzanne.obj"
```
With only this on the stack, calling `M~RenderMeshPreview` will yield an image result.
...
- It is recommended to define aliases such as `RC` and `RO` at the top of your file, but to remain explicit, these aliases will not be provided by the library
...
- Skim the generated [docs](https://dguto9.github.io/meshing-around/)
